...
<body>
        ...
        <div class="sidebar-menu w-full lg:w-[75px] shadow lg:flex lg:flex-col flex flex-row justify-between items-center fixed lg:relative z-40 bottom-0 bg-white dark:bg-zinc-600 ">
            <div class="hidden lg:my-5 lg:block">
                <a href="index.html" class="block dark:hidden">
                    <span>
                        <img src="{{asset('images/logo.svg')}}" alt="" class="h-[30px]">
                    </span>
                </a>
                ...
                --}}
                    ...
                    <li class="relative lg:mt-4 dropdown lg:dropup">
                        <a href="#" class="dropdown-toggle" id="dropdownButton2" data-bs-toggle="dropdown">
                            <div>
                                @if(auth()->user()->avatar)
                                <img src="{{asset('images/avatar-2.jpg')}}" alt="user-img" class="mx-auto rounded-full w-9 h-9">
                                @else
                        ...
                        <ul class="absolute z-40 hidden float-left w-40 py-2 mx-4 mb-12 text-left list-none bg-white border-none rounded-lg shadow-lg dropdown-menu bg-clip-padding dark:bg-zinc-700" aria-labelledby="dropdownButton2">
                            <li>
                                <a class="block w-full px-4 py-2 text-sm font-normal text-gray-700 bg-transparent dropdown-item whitespace-nowrap hover:bg-gray-100/30 dark:text-gray-100 dark:hover:bg-zinc-600/50 ltr:text-left rtl:text-right" href="{{route('profile.edit')}}">Profile <i class="text-gray-500 rtl:float-left ltr:float-right ri-profile-line text-16"></i></a>
                            </li>
                            {{--
                        ...
                        --}}
                            ...
                            <li>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="block w-full px-4 py-2 text-sm font-normal text-gray-700 bg-transparent dropdown-item whitespace-nowrap hover:bg-gray-100/30 dark:text-gray-100 dark:hover:bg-zinc-600/50 ltr:text-left rtl:text-right">Log out <i class="text-gray-500 rtl:float-left ltr:float-right ri-logout-circle-r-line text-16"></i></button>
                                </form>
                            </li>
                        </ul>
                    </li>
    ...
    --}}
        ...
        <div class="chat-leftsidebar lg:w-[380px] group-data-[theme-color=violet]:bg-slate-50 group-data-[theme-color=green]:bg-green-50/20 group-data-[theme-color=red]:bg-red-50/20 shadow overflow-y-hidden mb-[80px] lg:mb-0 group-data-[theme-color=violet]:dark:bg-zinc-700 group-data-[theme-color=green]:dark:bg-zinc-700 group-data-[theme-color=red]:dark:bg-zinc-700" style="height:100vh;">
            <div>
                ...
                <div class="tab-content">
                    ...
                    <div>
                        ...
                        <div class="p-6 text-center border-b border-gray-100 dark:border-zinc-600">
                            <div class="mb-4">
                                <img src="{{asset('images/avatar-1.jpg')}}" class="w-24 h-24 p-1 mx-auto border border-gray-100 rounded-full dark:border-zinc-800" alt="">
                            </div>
                            ...
                            <div class="owl-carousel owl-theme" id="user-status-carousel">
                                ...
                                <div class="text-center">
                                    <a href="#" class="block p-2 mt-4 rounded group-data-[theme-color=violet]:bg-slate-100 group-data-[theme-color=green]:bg-green-50 group-data-[theme-color=red]:bg-red-50 group-data-[theme-color=violet]:dark:bg-zinc-600 group-data-[theme-color=green]:dark:bg-zinc-600 group-data-[theme-color=red]:dark:bg-zinc-600">
                                        ...
                                        <h5 class="mt-4 mb-0 truncate text-13 dark:text-gray-50">{{$user->name}}</h5>
                                    </a>
                                </div>
                                @endforeach
                        ...
                        <div>
                            ...
                            <div class="h-[610px] px-2" data-simplebar>
                                ...
                                <ul class="chat-user-list">
                                    @foreach(auth()->user()->rooms->sortByDesc(function ($room) {
                                        return $room->messages()->latest()->first()?->created_at;
                                        }); 
                                    as $room)
                                    <li class="px-5 py-[15px] group-data-[theme-color=violet]:hover:bg-slate-100 group-data-[theme-color=green]:hover:bg-green-50/50 group-data-[theme-color=red]:hover:bg-red-50/50 transition-all ease-in-out border-b border-white/20 dark:border-zinc-700 group-data-[theme-color=violet]:dark:hover:bg-zinc-600 group-data-[theme-color=green]:dark:hover:bg-zinc-600 group-data-[theme-color=red]:dark:hover:bg-zinc-600 dark:hover:border-zinc-700">
                                        <a href="{{ route('chatRooms.show', $room) }}">
                                            <div class="flex">
                                                <div class="ltr:mr-3 rtl:ml-3">
                                                    @if($room->users()->where('id', '!=', auth()->user()->id)->first()->avatar)
                                                    <img src="{{asset('images/avatar-2.jpg')}}" class="rounded-full w-9 h-9" alt="">
                                                ...
                                                <div class="flex-grow overflow-hidden">
                                                    <h5 class="mb-1 text-base truncate dark:text-gray-50">{{ $room->users()->where('id', '!=', auth()->user()->id)->first()->name }}</h5>
                                                    <p class="mb-0 text-gray-500 truncate dark:text-gray-300 text-14" id="latest-message">{{$room->messages()->latest()->first()?->sender->is(auth()->user()) ? 'You: ' : ''}} {{$room->messages()->latest()->first()?->text ?: 'Say hi to your friend! 👋'}}</p>
                                                </div>
                                                <div class="text-gray-500 text-11 dark:text-gray-300" id="latest-message-date">{{$room->messages()->latest()->first()?->created_at->format('H:i, M d')}}</div>
                                            </div>
                                        </a>
                                    </li>
                                    @endforeach
                                    ...
                                    <li class="unread px-5 py-[15px] group-data-[theme-color=violet]:hover:bg-slate-100 group-data-[theme-color=green]:hover:bg-green-50/50 group-data-[theme-color=red]:hover:bg-red-50/50 transition-all ease-in-out border-b border-white/20 dark:border-zinc-700 group-data-[theme-color=violet]:dark:hover:bg-zinc-600 group-data-[theme-color=green]:dark:hover:bg-zinc-600 group-data-[theme-color=red]:dark:hover:bg-zinc-600 dark:hover:border-zinc-700">
                                        <a href="#">
                                            <div class="relative flex">
                                                <div class="relative self-center ltr:mr-3 rtl:ml-3">
                                                    <img src="{{asset('images/avatar-3.jpg')}}" class="rounded-full w-9 h-9" alt="">
        ...
        @if($chatRoom)
        <!-- Start User chat -->
        <x-chat-component :room="$chatRoom" :userTo="$chatRoom->users()->where('id', '!=', auth()->user()->id)->first()" />
        <!-- End User chat -->
        @else
